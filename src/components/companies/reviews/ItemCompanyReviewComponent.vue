...
import type { isRecommendReviewResponse as isRecommendReviewResponseInterface } from "@/types/Review";
...
// Cria uma cópia local reativa da review
...
const hasHalfStar = computed<boolean>(
    () => (localReview.score ?? 0) % 1 >= 0.5,
);
const emptyStars = computed<number>(
    () => 5 - filledStars.value - (hasHalfStar.value ? 1 : 0),
);
...
const truncatedNegativeComment = computed(() => {
    ...
    if (isExpandedNegativeComment.value || comment.length <= 320)
        return comment;
...
const truncatedPositiveComment = computed(() => {
    ...
    if (isExpandedPositiveComment.value || comment.length <= 320)
        return comment;
    return comment.slice(0, 320) + "...";
});
...
const handleUsefulReview = async (isRecommend: boolean) => {
    try {
        ...
        if (response.success) {
            localReview.usefulRate.useful =
                response.data.usefulRate?.useful ||
                localReview.usefulRate.useful + (isRecommend ? 1 : 0);
...
<template>
    <div
        class="mt-12 md:mt-8 flex md:flex-row flex-col gap-4 border-b-[3px] pb-10 border-[#E4E2E0]"
    >
        <div class="">
            ...
            <div class="flex mt-2">
                <i
                    ...
                    :key="'filled-' + n"
                    class="bx bxs-star text-2xl text-[#003F72]"
                ></i>
                <i
                    v-if="hasHalfStar"
                    class="bx bxs-star-half text-[#003F72] text-2xl"
                ></i>
                <i
                    v-for="n in emptyStars"
                    :key="'empty-' + n"
                    class="bx bx-star text-2xl text-[#003F72]"
                ></i>
            </div>
        </div>
        <div class="">
            <div class="pb-5">
                <h2 class="text-2xl pr-2 font-bold dark:text-white">
                    {{ localReview.reviewTitle }}
                </h2>
                <div class="my-1">
                    <span
                        class="text-[#767676] underline md:text-normal text-sm dark:text-gray-300"
                    >
                        {{ localReview.jobTitle }}
                    </span>
                    <span class="text-[#767676] dark:text-gray-300">
                        {{
                            localReview.employeeType === "Ex-employee"
                                ? " (Ex-Funcionário) "
                                : " (Funcionário actual) "
                        }}
                    </span>
                    <span
                        class="md:inline-block ml-0 md:ml-2 md:text-normal text-sm text-[#767676] underline dark:text-gray-300"
                        >da {{ currentCompany.name }}</span
                    >
                    <span class="text-md text-[#767676] dark:text-gray-300">
                        em {{ getDay }} de {{ getMonth }} de {{ getYear }}</span
                    >
                </div>
                <h3 class="mt-3 text-[#767676] md:pr-0 pr-2 dark:text-gray-300">
                    {{ localReview.comment?.comment }}
                </h3>
            </div>
            ...
            <div>
                ...
                <span class="md:pr-0 pr-4 dark:text-gray-300">{{ truncatedNegativeComment }}
                    <span
                        v-if="localReview.comment?.negativeAspects.length > 320"
                        @click="toggleReadMoreNegativeComment"
                        class="text-[#003f72] underline cursor-pointer dark:text-blue-500">
                        {{
                            isExpandedNegativeComment
                                ? "mostrar menos"
                                : "saiba mais..."
                        }}
                    </span>
            ...
            <div class="my-5">
                ...
                <span class="md:pr-0 pr-4 dark:text-gray-300">{{ truncatedPositiveComment }}
                    <span
                        v-if="localReview.comment?.positiveAspects.length > 320"
                        @click="toggleReadMorePositiveComment"
                        class="text-[#003f72] underline cursor-pointer dark:text-blue-500">
                        {{
                            isExpandedPositiveComment
                                ? "mostrar menos"
                                : "saiba mais..."
                        }}
                    </span>
                ...
                <div v-else class="">
                    <div
                        ...
                        <button
                            ...
                            class="flex gap-2 items-center mr-4 hover:text-[#003f72] transition-colors dark:text-gray-300">
                            <img class="" src="@/assets/img/share.svg" alt="" />
                            Compartilhar
                        </button>
                    </div>
                        <div v-if="!isUserUseFulReview" class="bg-gray-50 p-4 rounded-lg mt-4">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <span class="text-[#595959] font-medium dark:text-gray-300"
                                >Esta avaliação foi útil?</span
                            >
                            <button
                                v-if="localReview.usefulRate.useful === 0"
                                type="button"
                                @click="openShareModal"
                                class="flex gap-2 items-center sm:ml-auto text-[#003f72] hover:text-[#002a4d] transition-colors self-end sm:self-auto dark:text-gray-300"
                            >
                                <img
                                    class="w-5 h-5"
