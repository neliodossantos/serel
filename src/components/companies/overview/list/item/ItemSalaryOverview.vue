...
<template>
  <div class="border shadow p-5 rounded mt-3">
    ...
    <div class="">
      ...
      <div class="">
        <div class="">
          <div class="flex md:justify-start justify-between lg:gap-3 flex-wrap">
            <div class="flex items-center justify-between gap-3 md:flex-col md:gap-0 md:items-start flex-row">
              <p class="font-bold xl:text-lg dark:text-white"> {{currentCurrency.simbol}} {{ formatSalary(convertedSalary.highestSalary ? convertedSalary.highestSalary : 0,
                  { decimalPlaces: 2, currency: true, decimal: true}) }}</p>
              <span class="text-sm flex mx-auto items-center dark:text-white">
                <i class='bx bx-down-arrow-alt text-red-500 text-lg' ></i>Sálario Baixo
              </span>
            </div>
            <span class="font-bold xl:text-lg 2xl:inline hidden dark:text-white">-</span>
            <div class="flex items-center justify-between gap-3 md:flex-col md:gap-0 md:items-start flex-row dark:text-white">
              <p class="font-bold xl:text-lg"> {{currentCurrency.simbol}} {{ formatSalary(convertedSalary.highestSalary ? convertedSalary.highestSalary : 0,
                  { decimalPlaces: 2, currency: true, decimal: true}) }}</p>
              <span class="text-sm flex mx-auto items-center dark:text-white">
                  <i class='bx bx-up-arrow-alt text-green-500 text-lg'></i>Sálario Alto
              </span>
            </div>
          </div>
        </div>
