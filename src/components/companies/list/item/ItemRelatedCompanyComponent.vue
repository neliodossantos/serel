...
function goToOverviewCompany(){
  window.location.href = "/overview/company/" + props.company.id;
}
...
<template>
  <div class="w-full h-full pb-10 border-b-2 sm:border-none sm:pb-0">
    <div class="flex md:flex-col md:justify-between gap-5 h-full">
      <img @click="goToOverviewCompany()" :src="company.logo" class="xxs:w-16 xxs:h-16 w-24 h-24 2xl:w-32 2xl:h-32 3xl:w-36 3xl:h-36 hover:cursor-pointer object-cover rounded" alt="">
      <div class="flex flex-col justify-between gap-1 h-full flex-1">
        <div class="max-w-full">
          <h3 class="xs:max-w-36 text-md 3xl:text-lg max-w-44 md:max-w-32 lg:max-w-44 2xl:max-w-48 3xl:max-w-60 hover:underline truncate cursor-pointer dark:text-white" @click="goToOverviewCompany()">{{company.name}}</h3>
          <span class="flex gap-1 items-center font-light text-sm 3xl:text-base dark:text-white">
        ...
        <span class="font-light text-sm 3xl:text-base dark:text-white">{{company.sector}}</span>
      </div>
    </div>
  </div>
</template>
