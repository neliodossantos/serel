...
function goToOverviewCompany(id:string){
...
<template>
  <div class="w-full pb-10 border-b-2 sm:border-none sm:pb-0">
    <div class="flex gap-5">
      ...
      <div class="flex flex-col justify-between w-full">
        <div class="max-w-full">
          <h3 class="xs:max-w-36 text-md 3xl:text-lg max-w-44 md:max-w-32 lg:max-w-44 2xl:max-w-48 3xl:max-w-60 hover:underline truncate cursor-pointer text-white" @click="goToOverviewCompany(company.id)">{{company.name}}</h3>
          <span class="flex gap-1 items-center font-light text-sm 3xl:text-base dark:text-gray-300">
        ...
        <span class="font-light text-sm 3xl:text-base dark:text-gray-300">{{company.sector}}</span>
      </div>
    </div>
  </div>
</template>
