...
import CustomSelectComponent from "@/components/select/SelectCompanyComponent.vue";
...
watch(() => review.companyId, () => { errors.companyId = validators.validateField(review.companyId, 'Empresa'); },{ deep: true});
...
watch(() => review.reviewTitle , () => { errors.reviewTitle = validators.validateReviewTitle(review.reviewTitle)} , {deep: true});
...
const validateForm = () => {
  errors.companyId = validators.validateField(review.companyId, 'Empresa');
  ...
  errors.reviewTitle = validators.validateReviewTitle(review.reviewTitle)
...
onMounted(async () => {
  try {
    const response = await companyStore.getAll({ filter:{} , page: 1 , limit : 100 });
...
const handleSelect = (company: Company | null) => {
  review.companyId = company?.id || undefined;
}
    ...
    <div class="py-10 sm:py-5">
      ...
      <div @submit.prevent="handleSubmit" class="md:rounded-lg md:border-2 md:border-[#C3C6E5] flex flex-col gap-12 md:px-8 md:py-10">
        <div class="">
          ...
          <CustomSelectComponent :items="companies" placeholder="Selecione uma empresa..."
              @select="handleSelect" @reportMissing="reportMissingCompany" @clear="value => review.companyId = ''"/>
        ...
        <div class="">
          ...
          <div class="flex justify-between w-full text-[#767676] dark:text-gray-300 mt-2">
            <span class="font-medium">Pessima</span>
