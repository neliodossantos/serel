...
import SelectCompanyComponent from "@/components/select/SelectCompanyComponent.vue";
...
onMounted(async () => {
  try {
    const [responseCompany, responseArea] = await Promise.all([
      companyStore.getAll({ filter: {}, page: 1, limit: 100 }),
      areaStore.getAll({ filter: {}, page: 1, limit: 100 }),
    ]);
    companies.value = responseCompany.data?.companies ?? [];
    areas.value = responseArea.data ?? [];
  }catch (e) {
    toast.error('Ocorreu um erro ao buscar os dados das areas');
  }
});

const handleSelectCompany = (company: Company | null) => {
  salary.companyId = company?.id || undefined;
}
...
const fetchRole =  async (areaId: string | undefined) => {
  try {
    ...
    toast.error('Ocorreu um erro ao buscar os dados de funções');
  }
}
...
const reportMissingArea = (value: boolean) => {};
const reportMissingRole = (value: boolean) => {};
const reportMissingCompany = (value: boolean , name : string) => {
    window.location.href = "/add/company?name=" + name;
};
...
<template>
  <form @submit.prevent="handleSubmit">
    <div class="py-10 sm:py-5">
      <h2 class="md:text-2xl max-w-[800px] text-lg mb-6 dark:text-gray-300">Compartilhe sua avaliação anônima sobre a empresa.</h2>
      <div class="md:rounded-lg md:border-2 md:border-[#C3C6E5] flex flex-col gap-12 md:px-8 md:py-10">
        <div class="">
          <h3 class="font-bold text-md sm:text-lg mb-2 dark:text-gray-300">Empresa</h3>
          <SelectCompanyComponent :items="companies" placeholder="Selecione uma empresa..."
                                 @select="handleSelectCompany" @reportMissing="reportMissingCompany" @clear="value => salary.companyId = ''"/>
          <span class="text-red-400 mt-1" v-if="!errors.companyId.valid">{{errors.companyId.message}}</span>
        </div>
        <div class="">
        ...
        <div>
          ...
          <SelectAreaComponent :items="areas" placeholder="Selecione a area..."
                                 @select="handleSelectArea" @reportMissing="reportMissingArea" @clear="clearAreaSelected"/>
        ...
        <div>
          ...
          <SelectRoleComponent :items="roles" placeholder="Selecione a função..."
                               @select="handleSelectRole" @reportMissing="reportMissingRole" @clear="value => selectedRole = ''"/>
        ...
        <div class="">
          ...
          <div class="relative w-full">
            <input
                ...
                placeholder="Digite o valor salarial"
            ...
            <div class="absolute h-full right-0 top-0">
              <div
                  class="w-full h-full flex gap-1 items-center justify-between p-2 rounded-r-lg text-black
                         focus:outline-none outline-none  border border-[#D4D2D0] dark:text-gray-900">
                  <i class='bx bx-coin text-2xl dark:text-gray-900'></i>AOA
              </div>
            </div>
          </div>
