...
<template>
  <div class="md:px-10">
    <div class="mb-3">
      <h1 class="font-[700] text-2xl">Já tem uma conta?</h1>
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="flex flex-wrap items-center justify-between">
        <span>Nenhuma conta? <a class="underline text-[#344054] cursor-pointer" @click="redirectToComponent('signup')">Crie uma!</a></span>
        <a class="underline text-[#344054] my-3 block cursor-pointer" href="/forgot-password">Esqueci a minha palavra-passe</a>
      </div>
      <div class="form-group mb-3">
        <button type="submit" class="flex items-center gap-3 justify-center mt-3 bg-[#003F72] text-white w-full py-[10px] px-[20px] rounded-lg font-bold" :class="{'disable' : isLoading }" :disabled="isLoading">
          {{ isLoading ? 'Processando...' : 'Iniciar a sessão'}}
