...
const selectedArea = ref<Area | null>(props.modelValue)
...
const selectItem = (item: Area | null) => {
...
const clearSelection = () => {
  ...
  isOpen.value = false
  highlightedIndex.value = -1
  emit('clear', selectedArea.value)
  emit('update:modelValue', null)
  inputRef.value?.focus()
}

const reportMissingArea = () => {
...
const scrollToHighlighted = () => {
  if (highlightedIndex.value >= 0) {
    nextTick(() => {
      const list = dropdownRef.value?.querySelector('.select-area-list')
...
const handleKeydown = (event: KeyboardEvent) => {
  if (!isOpen.value || selectedArea.value) return
    ...
    <div class="relative" ref="dropdownRef">
      <div class="relative">
        ...
        <div class="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
          ...
          <!-- Clear button -->
          <button 
            v-else-if="searchQuery || selectedArea" 
      ...
      <div 
        ...
        <ul 
          ...
          class="select-area-list max-h-64 overflow-y-auto"
        >
          <li 
        ...
        <div v-if="filteredItems.length > 0" class="px-4 py-2 bg-gray-50 border-t border-gray-100 text-xs text-gray-500 flex items-center justify-between rounded-b-lg">
          <span>{{ filteredItems.length }} de {{ props.items.length }} áreas</span>
...
.select-area-list::-webkit-scrollbar {
  width: 6px;
}

.select-area-list::-webkit-scrollbar-track {
  background: transparent;
}

.select-area-list::-webkit-scrollbar-thumb {
  background-color: rgba(156, 163, 175, 0.4);
  border-radius: 3px;
  transition: background-color 0.2s ease;
}

.select-area-list::-webkit-scrollbar-thumb:hover {
